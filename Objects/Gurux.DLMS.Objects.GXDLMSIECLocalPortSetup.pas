...
unit Gurux.DLMS.Objects.GXDLMSIECLocalPortSetup;
...
type
TGXDLMSIECLocalPortSetup = class(TGXDLMSObject)
...
constructor TGXDLMSIECLocalPortSetup.Create;
begin
  Create('', 0);
end;

constructor TGXDLMSIECLocalPortSetup.Create(ln: string);
begin
  Create(ln, 0);
end;

constructor TGXDLMSIECLocalPortSetup.Create(ln: string; sn: System.UInt16);
begin
  inherited Create(TObjectType.otIecLocalPortSetup, ln, 0);
  FVersion := 1;
end;

function TGXDLMSIECLocalPortSetup.GetValues() : TArray<TValue>;
begin
...
function TGXDLMSIECLocalPortSetup.GetAttributeIndexToRead(All: Boolean): TArray<Integer>;
var
  items : TList<Integer>;
begin
...
function TGXDLMSIECLocalPortSetup.GetAttributeCount: Integer;
begin
  Result := 9;
end;

function TGXDLMSIECLocalPortSetup.GetMethodCount: Integer;
begin
  Result := 0;
end;

function TGXDLMSIECLocalPortSetup.GetDataType(index: Integer): TDataType;
begin
  if (index = 1) then
  begin
...
function TGXDLMSIECLocalPortSetup.GetValue(e: TValueEventArgs): TValue;
begin
  if e.Index = 1 then
  begin
...
procedure TGXDLMSIECLocalPortSetup.SetValue(e: TValueEventArgs);
begin
  if e.Index = 1 then
  begin
...
function TGXDLMSIECLocalPortSetup.Invoke(e: TValueEventArgs): TBytes;
begin
