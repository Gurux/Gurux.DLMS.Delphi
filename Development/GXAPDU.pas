...
TGXAPDU = class
  ...
  class function ParseApplicationContextName(
      settings: TGXDLMSSettings;
      buff: TGXByteBuffer;
      xml: TGXDLMSTranslatorStructure): TApplicationContextName;
  ...
  class function ParsePDU(
      ...
      cipher: TGXCiphering;
      buff: TGXByteBuffer;
      xml: TGXDLMSTranslatorStructure): TValue; static;
...
begin
  ...
  if (tmp[0] <> $60) or
      ...
      begin
          ...
          Result := TApplicationContextName.Unknown;
          Exit;
      end
      else
  ...
  begin
      if BYTE(TGXDLMSAssociationLogicalName(settings.GetAssignedAssociation()).ApplicationContextName.ContextId) = name Then
      begin
          Result := TApplicationContextName.Unknown;
          Exit;
      end;
      Result := TGXDLMSAssociationLogicalName(settings.GetAssignedAssociation()).ApplicationContextName.ContextId;
      Exit;
  end;
  if settings.UseLogicalNameReferencing Then
    if (name = BYTE(TApplicationContextName.LogicalName)) and ((settings.Cipher = Nil) or (settings.Cipher.Security = TSecurity.None)) Then
    begin
        Result := TApplicationContextName.Unknown;
        Exit;
    end;
    // If ciphering is used.
    if (name = BYTE(TApplicationContextName.LogicalNameWithCiphering)) and ((settings.Cipher <> Nil) and (settings.Cipher.Security <> TSecurity.None)) Then
    begin
        Result := TApplicationContextName.Unknown;
        Exit;
    end
  else
    if (name = BYTE(TApplicationContextName.ShortName)) and ((settings.Cipher = Nil) or (settings.Cipher.Security = TSecurity.None)) Then
    begin
        Result := TApplicationContextName.Unknown;
        Exit;
    end;
    // If ciphering is used.
    if (name = BYTE(TApplicationContextName.ShortNameWithCiphering)) and ((settings.Cipher <> Nil) and (settings.Cipher.Security <> TSecurity.None)) Then
    begin
        Result := TApplicationContextName.Unknown;
        Exit;
    end;
    Result := TApplicationContextName(name);
end;
...
class function TGXAPDU.ParsePDU(
    ...
    cipher: TGXCiphering;
    buff: TGXByteBuffer;
    xml: TGXDLMSTranslatorStructure): TValue;
var
  size: Integer;
  len, tag: Byte;
begin
...
var
  ...
  len, tag: Byte;
  tmp: TBytes;
  name: TApplicationContextName;
  msg : string;
begin
  resultComponent := TAssociationResult.Accepted;
  Result := TValue.From(TSourceDiagnostic.None);
  ...
  begin
      ...
      begin
        name := ParseApplicationContextName(settings, buff, xml);
        if name <> TApplicationContextName.Unknown Then
        begin
         if settings.IsServer Then
          begin
            Result := TValue.From(name);
            Exit;
          end;
        case name of
            TApplicationContextName.LogicalName:
               msg := ' Meter expects Logical Name referencing.';
            TApplicationContextName.ShortName:
                msg := ' Meter expects Short Name referencing.';
            TApplicationContextName.LogicalNameWithCiphering:
                msg := ' Meter expects Logical Name referencing with secured connection.';
            TApplicationContextName.ShortNameWithCiphering:
      ...
      begin
        ...
        if (resultComponent <> TAssociationResult.Accepted) and
            (Result.AsType<TSourceDiagnostic>() <> TSourceDiagnostic.None) Then
          raise TGXDLMSException.Create(resultComponent, Result.AsType<TSourceDiagnostic>());
...
class procedure TGXAPDU.GenerateAARE(settings: TGXDLMSSettings; data: TGXByteBuffer;
  result: TAssociationResult; diagnostic: TValue; cipher: TGXCiphering;
  errorData: TGXByteBuffer; encryptedData: TGXByteBuffer);
var
  offset: Integer;
  tmp: TBytes;
  tmp2: TGXByteBuffer;
begin
    ...
    data.SetUInt8(5); //len
    //Tag
    if diagnostic.IsType<TSourceDiagnostic> then
      data.SetUInt8($A1)
    else
      data.SetUInt8($A2);
    data.SetUInt8(3); //len
    ...
    // Choice for result (INTEGER, universal)
    data.SetUInt8(1); //Len
    data.SetUInt8(Byte(diagnostic.AsType<TSourceDiagnostic>)); //diagnostic
    ...
    if (result = TAssociationResult.Accepted) or (cipher = Nil) or (cipher.Security = TSecurity.None) Then
    begin
      ...
      begin
        ...
        try
         ...
         if (errorData <> Nil) and (errorData.Size <> 0) Then
            tmp := errorData.ToArray()
         else
