...
uses SysUtils, Rtti, Math,
Gurux.DLMS.DataType, Gurux.DLMS.ObjectType, Gurux.DLMS.Authentication,
Gurux.DLMS.Priority, Gurux.DLMS.ServiceClass, Gurux.DLMS.InterfaceType,
Gurux.DLMS.GXHdlcSettings, Gurux.DLMS.RequestTypes, Gurux.DLMS.Command,
...
Gurux.DLMS.PlcDataLinkData,
Gurux.DLMS.Plc.Enums.PlcMacSubframes;
...
type
  ...
  class function UseHdlc(value: TInterfaceType): BOOLEAN;
  ...
  class function GetMacFrame(settings: TGXDLMSSettings; frame: BYTE; creditFields: BYTE; data: TGXByteBuffer): TBytes;
  // Get MAC LLC frame for data.
  class function GetPlcFrame(settings: TGXDLMSSettings; creditFields: BYTE; data: TGXByteBuffer): TBytes;
  // Get MAC HDLC frame for data.
  class function GetMacHdlcFrame(settings: TGXDLMSSettings; frame: BYTE; creditFields: BYTE; data: TGXByteBuffer): TBytes;
  ...
  class function GetWrapperFrame(settings: TGXDLMSSettings; command: TCommand; data: TGXByteBuffer): TBytes;static;
 ...
 // Get physical and logical address from server address.
  ...
  class procedure ParseSnrmUaResponse(Data : TGXByteBuffer; settings: TGXDLMSSettings);
end;
...
class function TGXDLMS.UseHdlc(value: TInterfaceType): BOOLEAN;
begin
   Result := (value = TInterfaceType.HDLC) or
                (value = TInterfaceType.HdlcWithModeE) or
                (value = TInterfaceType.PlcHdlc);
end;
...
begin
  ...
  if UseHdlc(p.Settings.InterfaceType) Then
      ...
      begin
        ...
        begin
            if p.Settings.InterfaceType = TInterfaceType.WRAPPER Then
                list.Add(GetWrapperFrame(p.Settings, p.Command, bb))
...
// Split DLMS PDU to wrapper frames.
class function TGXDLMS.GetWrapperFrame(settings: TGXDLMSSettings; command: TCommand; data: TGXByteBuffer): TBytes;
var
  bb: TGXByteBuffer;
begin
  bb := TGXByteBuffer.Create();
  try
    ...
    begin
      ...
      if (settings.PushClientAddress <> 0) and
         ((command = TCommand.DataNotification) or (command = TCommand.EventNotification)) then
...
begin
  ...
  try
    ...
    frameSize := Integer(settings.HDLC.MaxInfoTX);
...
class function TGXDLMS.GetMacFrame(settings: TGXDLMSSettings; frame: BYTE; creditFields: BYTE; data: TGXByteBuffer): TBytes;
begin
    if settings.InterfaceType = TInterfaceType.Plc Then
        Result := GetPlcFrame(settings, creditFields, data)
    else
    Result := GetMacHdlcFrame(settings, frame, creditFields, data);
end;

class function TGXDLMS.GetPlcFrame(settings: TGXDLMSSettings; creditFields: BYTE; data: TGXByteBuffer): TBytes;
var
   val, frameSize, padLen: Integer;
   bb: TGXByteBuffer;
   crc: UInt16;
begin
    frameSize := data.Available;
    //Max frame size is 134 bytes.
    if frameSize > 134 then
        frameSize := 134;

    //PAD Length.
    padLen := (36 - ((11 + frameSize) Mod 36)) Mod 36;
    bb := TGXByteBuffer.Create;
    ...
    bb.SetUInt8((settings.Plc.MacSourceAddress shr 4));
    val := settings.Plc.MacSourceAddress shl 12;
    val := val or settings.Plc.MacDestinationAddress and $FFF;
    bb.SetUInt16(val);
    bb.SetUInt8(padLen);
    //Control byte.
    bb.SetUInt8(BYTE(TPlcDataLinkData.Request));
    bb.SetUInt8(settings.ServerAddress);
    bb.SetUInt8(settings.ClientAddress);
    bb.SetArray(data, frameSize);
    //Add padding.
    while (padLen <> 0) do
    begin
        bb.SetUInt8(0);
    ...
    //Checksum.
    crc := TGXFCS16.CountFCS16(bb.GetData, 0, bb.Size);
    bb.SetUInt16(crc);
    //Remove sent data in server side.
    if settings.IsServer then
    begin
...
// Get MAC HDLC frame for data.
class function TGXDLMS.GetMacHdlcFrame(settings: TGXDLMSSettings; frame: BYTE; creditFields: BYTE; data: TGXByteBuffer): TBytes;
var
  bb: TGXByteBuffer;
  padLen, val: Integer;
  tmp: TBytes;
  crc: UInt32;
begin
    if settings.Hdlc.MaxInfoTX > 126 Then
    begin
    ...
    val := val or settings.Plc.MacDestinationAddress and $FFF;
    bb.SetUInt16(val);
    tmp := TGXDLMS.GetHdlcFrame(settings, frame, data);
    padLen := (36 - ((10 + Length(tmp)) Mod 36)) Mod 36;
    bb.SetUInt8(padLen);
    bb.SetArray(tmp);
    //Add padding.
    while (padLen <> 0) do
    ...
    //Checksum.
    crc := TGXFCS16.CountFCS24(bb.GetData(), 2, bb.Size - 2 - padLen);
    bb.SetUInt8(crc shr 16);
    bb.SetUInt16(crc);
    //Add NC
    val := Floor(bb.Size / 36);
    if bb.Size Mod 36 <> 0 Then
    begin
      val := val + 1;
    end;
    case val of
        1: val := Integer(TPlcMacSubframes.One);
        2: val := Integer(TPlcMacSubframes.Two);
        3: val := Integer(TPlcMacSubframes.Three);
        4: val := Integer(TPlcMacSubframes.Four);
        5: val := Integer(TPlcMacSubframes.Five);
        6: val := Integer(TPlcMacSubframes.Six);
        7: val := Integer(TPlcMacSubframes.Seven);
    else raise EArgumentException.Create('Data length is too high.');
    end;
    bb.SetUInt16(0, val);
    Result := bb.ToArray();
end;
...
begin
  ...
  begin
    //If not notify.
    if Not ((reply.Position < reply.Size) and
      ((reply.GetUInt8(reply.Position) = $13) or (reply.GetUInt8(reply.Position) = $3))) Then
    begin
      //If echo.
  ...
  moreData := (frame and $8) <> 0;
  // Get frame type.
  frame := reply.GetUInt8();
  if (data.xml = Nil) and Not settings.CheckFrame(frame, data.xml) Then
  begin
  ...
  if ((frame = $13) or (frame = $3)) and (Not isNotify) and (notify <> Nil) Then
  begin
      isNotify := true;
      notify.ClientAddress := target;
      notify.ServerAddress := source;
  ...
  if (frame <> $13) and (frame <> $3) and ((frame and Integer(THdlcFrameType.Uframe)) = Integer(THdlcFrameType.Uframe)) Then
  begin
  ...
  else if (frame <> $13) and (frame <> $3) and ((frame and Integer(THdlcFrameType.Sframe)) = Integer(THdlcFrameType.Sframe)) Then
  begin
      // If S-frame
...
begin
  ...
  begin
    ...
    begin
        ...
        else if (settings.ClientAddress = 0) and (settings.ServerAddress = $7F) Then
          Result := true
        Else
          Result := False;
        Exit;
...
begin
  ...
  if (settings.InterfaceType = TInterfaceType.HDLC) or (settings.InterfaceType = TInterfaceType.HdlcWithModeE) then
  begin
    frame := GetHdlcData(settings.isServer, settings, reply, data, notify);
    isLast := (frame and $10) <> 0;
    if (notify <> Nil) and ((frame = $13) or (frame = $3)) Then
    begin
      data := notify;
      Result := False;
    end;
  ...
  if ((data.Xml <> Nil) or (((frame <> $13) and (frame <> $3)) or data.IsMoreData) and ((frame and $1) <> 0)) Then
  begin
   if (frame = $3) and data.IsMoreData Then
      begin
          tmp := GetData(settings, reply, data, notify);
          data.Data.Position := 0;
          Result := tmp;
      end
      else
  ...
  if (frame = $13) and Not data.IsMoreData then
    data.Data.Position := 0;
...
class procedure TGXDLMS.ParseSnrmUaResponse(Data : TGXByteBuffer; settings: TGXDLMSSettings);
var
  len: byte;
  val : Variant;
  id : THDLCInfo;
begin
  ...
  if data.Size = 0 Then
  begin
    settings.Hdlc.MaxInfoRX := 128;
    settings.Hdlc.MaxInfoTX := 128;
    settings.Hdlc.WindowSizeRX := 1;
    settings.Hdlc.WindowSizeTX := 1;
  end
  else
  begin
    ...
    begin
        ...
        // reversed to ours
        case id of
          THDLCInfo.MaxInfoTX: settings.HDLC.MaxInfoRX := val;
          HDLCInfo.MaxInfoRX: settings.HDLC.MaxInfoTX := val;
          HDLCInfo.WindowSizeTX: settings.HDLC.WindowSizeRX := val;
          HDLCInfo.WindowSizeRX: settings.HDLC.WindowSizeTX := val;
        else
          raise TGXDLMSException.Create('Invalid UA response.');
        end;
    end;
  end;
  settings.Connected := TConnectionState.csHdlc;
end;
