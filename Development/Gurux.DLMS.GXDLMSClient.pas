...
TGXDLMSClient = class (TInterfacedObject)
  ...
  private
    ...
    function get_Hdlc: TGXHDLCSettings;
    ...
    function GetGbtWindowSize: BYTE;
    procedure SetGbtWindowSize(AValue: BYTE);
  ...
  public
    ...
    property HdlcSettings: TGXHDLCSettings read get_Hdlc;
    ...
    property GBTWindowSize : Byte read GetGBTWindowSize write SetGBTWindowSize;
    ...
    class function EncryptLandisGyrHighLevelAuthentication(password: TBytes; seed: TBytes): TBytes;
end;
...
function TGXDLMSClient.get_Hdlc: TGXHDLCSettings;
begin
  Result := FSettings.HDLC;
end;
...
begin
  ...
  try
    ...
    if DEFAULT_MAX_INFO_TX <> FSettings.HDLC.MaxInfoTX Then
    begin
      data.SetUInt8(BYTE(THDLCInfo.MaxInfoTX));
      TGXDLMS.AppendHdlcParameter(data, FSettings.HDLC.MaxInfoTX);
    end;
    if DEFAULT_MAX_INFO_RX <> FSettings.HDLC.MaxInfoRX Then
    begin
      data.SetUInt8(BYTE(THDLCInfo.MaxInfoRX));
      TGXDLMS.AppendHdlcParameter(data, FSettings.HDLC.MaxInfoRX);
     end;
    if DEFAULT_WINDOWS_SIZE_TX <> FSettings.HDLC.WindowSizeTX Then
    begin
      data.SetUInt8(BYTE(THDLCInfo.WindowSizeTX));
      data.SetUInt8($04);
      data.SetUInt32(FSettings.HDLC.WindowSizeTX);
     end;
    if DEFAULT_WINDOWS_SIZE_RX <> FSettings.HDLC.WindowSizeRX Then
    begin
      data.SetUInt8(BYTE(THDLCInfo.WindowSizeRX));
      data.SetUInt8($04);
      data.SetUInt32(FSettings.HDLC.WindowSizeRX);
     end;
...
begin
  ...
  begin
    ...
    begin
        ...
        // reversed to ours
        case id of
          THDLCInfo.MaxInfoTX: HdlcSettings.MaxInfoRX := val;
          HDLCInfo.MaxInfoRX: HdlcSettings.MaxInfoTX := val;
          HDLCInfo.WindowSizeTX: HdlcSettings.WindowSizeRX := val;
          HDLCInfo.WindowSizeRX: HdlcSettings.WindowSizeTX := val;
        else
...
begin
  ret := TGXAPDU.ParsePDU(FSettings, FSettings.Cipher, Data, Nil).AsType<TSourceDiagnostic>();
...
begin
 ...
 if ((FSettings.Authentication <> TAuthentication.atHighECDSA) and
  ...
  if (ManufacturerId = 'LGZ') and (FSettings.Authentication = TAuthentication.atHigh) Then
  begin
      challenge := EncryptLandisGyrHighLevelAuthentication(FSettings.Password, FSettings.StoCChallenge);
      tmp := TValue.From(challenge);
      if UseLogicalNameReferencing Then
...
begin
  ...
  try
    ...
    begin
      ...
      comp.Parent := FSettings.Objects;
      if ((onlyKnownObjects and (comp.ClassType = TGXDLMSObject)) or (ignoreInactiveObjects and (comp.LogicalName = '0.0.127.0.0.0'))) then
        FreeAndNil(comp)
      else
        FSettings.Objects.Add(comp);
    end;
...
function TGXDLMSClient.GetGbtWindowSize: BYTE;
begin
  Result := FSettings.GBTWindowSize;
end;

procedure TGXDLMSClient.SetGbtWindowSize(AValue: BYTE);
begin
  FSettings.GBTWindowSize := AValue;
end;

// Encrypt Landis+Gyr High level password.
class function TGXDLMSClient.EncryptLandisGyrHighLevelAuthentication(password: TBytes; seed: TBytes): TBytes;
var
  pos: Integer;
begin
 SetLength(Result, Length(seed));
 System.Move(seed[0], Result[0], Length(seed));
 pos := 0;
